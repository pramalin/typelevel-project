...
class JobRoutes[F[_]: Concurrent: Logger: SecuredHandler] private (jobs: Jobs[F]) extends HttpValidationDsl[F] {
    ...
    case req @ PUT -> Root / UUIDVar(id) asAuthed user =>
      req.request.validate[JobInfo] { jobInfo =>
        jobs.find(id).flatMap {
          ...
          case _ =>
            Forbidden(FailureResponse("You can only update your own job"))
        }
      }
  }
    ...
    case req @ DELETE -> Root / UUIDVar(id) asAuthed user =>
      jobs.find(id).flatMap {
        ...
        case Some(job) if (user.owns(job) || user.isAdmin) =>
            jobs.delete(id) *> Ok()
        case _ =>
          Forbidden(FailureResponse("You can only delete your own job"))
      }
  }
  ...
  val routes = Router(
    "/jobs" -> (unautheredRoutes <+> authedRoutes)
  )
}
