...
object Doobie extends IOApp.Simple {
  ...
  object Students {
    def make[F[_]: MonadCancelThrow](xa: Transactor[F]): Students[F] = new Students[F] {
      ...
      def findAll: F[List[Student]] =
        sql"select ide, name from students".query[Student].to[List].transact(xa)
  ...
  val smallProgram = postgresResource.use { xa =>
    ...
    for {
      id <- studentsRepo.create("adi")
      adi <- studentsRepo.findById(id)
      _ <- IO.println(s"This is the new student created $adi" )
    } yield ()
  }
