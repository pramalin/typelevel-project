    ...
    override protected def renderFormContent(): List[Html[App.Msg]]  =
      if (!Session.isActive) renderInvalidContents()
         ...
         List(
            ...
            renderToggle("Remote",  "remote", "checkbox", true, _ => ToggleRemote),
            ...
            button(`class` := "form-submit-btn", `type` := "button", onClick(AttemptPostJob))("Post Job - $" + Constants.advertPriceUSD),
        )
