...
final case class JobPage(id: String, maybeJob: Option[Job] = None, status: Page.Status = Page.Status.LOADING) extends Page {
    import JobPage.*
    ...
    private def renderNoJobPage() =
        div(`class` := "container-fluid thr-rock")(
            div(`class` := "row jvm-jobs-details-top-card")(
              status.kind match {
                ...
                case Page.StatusKind.SUCCESS =>
                    h1("Something's wrong. Server is healthy but no job...")
              }
            )
        )
